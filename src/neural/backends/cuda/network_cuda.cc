...
static size_t getMaxAttentionBodySize(const MultiHeadWeights& weights, int N) {
  ...
  if (weights.encoder.size() > 0) {
    encoder_d_model = weights.encoder[0].mha.q_b.empty()
                          ? weights.encoder[0].mha.q_w.size() / embedding_op_size
                          : weights.encoder[0].mha.q_b.size();
    encoder_k_d_model = weights.encoder[0].mha.k_b.empty()
                            ? weights.encoder[0].mha.k_w.size() / embedding_op_size
                            : weights.encoder[0].mha.k_b.size();
